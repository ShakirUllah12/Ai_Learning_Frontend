  ...
  useEffect(() => {
    const fetchChatHistory = async () => {
      try {
        ...
        const historyData = await aiService.getChatHistory(documentId);
      ...
      <div className="flex-1 p-6 overflow-y-auto bg-linear-to-br from-slate-50/50 via-white/50 to-slate-50/50">
        ...
        {loading && (
          <div className="flex items-center gap-3 my-4">
            ...
            <div className="flex items-center gap-2  mx-4 py-3 rounded-2xl rounded-bl-md bg-white border border-slate-200/60">
      ...
      <div className="p-5 border-t border-slate-200/60 bg-white/80">
        <form className="flex items-center gap-3" onSubmit={handleSendMessage}>
          <input
            ...
            className="flex-1 h-12 px-4 border-2 border-slate-200 rounded-xl bg-slate-50/50 text-slate-900 placeholder-slate-400 text-sm font-medium transition-all duration-200 focus:outline-0 focus:border-emerald-500 focus:bg-white focus:shadow-lg focus:shadow-emerald-500/10"
            disabled={loading}
          />
